...
let PaymentWebhookService = class PaymentWebhookService {
    constructor(configService) {
        this.processedWebhooks = new Set(); // Para prevenir procesamiento duplicado
        this.webhookSecret = configService.get('PAYMENT_GATEWAY_WEBHOOK_SECRET') || 'webhook-secret';
    }
    /**
    ...
    async processPaymentWebhook(payload) {
        ...
        if (this.processedWebhooks.has(payload.transactionId)) {
            throw new common_1.ConflictException('This webhook has already been processed');
        }
        // Validar timestamp (no más de 5 minutos de antigüedad)
        const now = Date.now() / 1000;
        if (Math.abs(now - payload.timestamp) > 300) {
            throw new common_1.BadRequestException('Webhook timestamp is too old');
        }
